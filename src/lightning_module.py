...
class HandwritingClassifier(pl.LightningModule):
    def __init__(self, model, config, train_dataset, val_dataset,test_dataset=None):
        ...
        # Define metrics
        self.train_accuracy = torchmetrics.classification.Accuracy(task="multiclass", num_classes=self.config["model"]["num_classes"])  # train accuracy metric
        self.val_accuracy = torchmetrics.classification.Accuracy(task="multiclass", num_classes=self.config["model"]["num_classes"])  # val accuracy metric
        self.test_accuracy = torchmetrics.classification.Accuracy(task="multiclass", num_classes=self.config["model"]["num_classes"])  # test accuracy metric

        # Define confusion matrix metric for test
        self.confusion_matrix_metric = ConfusionMatrix(num_classes=self.config["model"]["num_classes"],task="multiclass")
    ...
    def on_test_epoch_end(self):
        ...
        if self.class_names:
            self.logger.experiment.log_confusion_matrix(labels=self.class_names, matrix=confusion_matrix,max_categories=len(self.class_names))
        else:
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode=self.config['scheduler']['mode'], factor=self.config['scheduler']['factor'], patience=self.config['scheduler']['patience'],
                                                                    verbose=True,threshold=self.config['scheduler']['threshold'],min_lr=self.config['scheduler']['min_lr']),
